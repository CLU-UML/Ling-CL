...
class LogisticCurriculum(nn.Module):
    ...
    def forward(self, loss, training_progress, diff_class, writer = None):
        ...
        # difflist = diff_class.tolist()
        # c1 = torch.tensor([self.cfg[c]['c1'] for c in difflist]).to(loss.device)
        # c2 = torch.tensor([self.cfg[c]['c2'] for c in difflist]).to(loss.device)
