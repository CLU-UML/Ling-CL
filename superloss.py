...
class SuperLoss(nn.Module):
    def __init__(self, C=3, lam=1, mode = 'avg', decay=0.9):
        super().__init__()
        self.mode = mode
        if self.mode == 'avg+ent':
            self.register_buffer('sum', None)
            self.register_buffer('count', torch.tensor([0.,0.,0.]))
            self.tau = torch.zeros(3, dtype = torch.float)
        elif self.mode == 'avg':
            self.register_buffer('sum', None)
            self.register_buffer('count', torch.tensor(0.))
            self.tau = None
        elif self.mode == 'logc':
            self.tau = torch.log(torch.tensor(C))
        elif self.mode == 'expavg':
            self.register_buffer('avg', torch.tensor(0.))
            self.decay = decay
            self.tau = None
        self.lambertw = LambertW(lam)
                  
    def forward(self, loss, entropy_class):
        loss = loss.detach()
        ...
        elif self.mode == 'avg+ent':
            if self.sum is None:
                self.sum = torch.zeros(3, dtype = torch.float).to(loss.device)
                for i in range(3):
                    self.tau[i] = loss[entropy_class == i].mean()
            else:
                for i in range(3):
                    self.tau[i] = self.sum[i]/self.count[i]

            tau = torch.gather(self.tau, 0, entropy_class).to(loss.device)
        elif self.mode == 'expavg':
            if self.avg is None:
                self.avg = loss.mean()
            else:
                self.avg = self.decay * self.avg + (1 - self.decay) * loss.mean()
            tau = self.avg
        elif self.mode == 'logc':
            tau = self.tau

        conf = self.lambertw(loss - tau)
        ...
        if self.mode == 'avg':
            ...
            for i in range(3):
                self.sum[i] += loss_w[entropy_class == i].sum()
                self.count[i] += conf[entropy_class == i].sum()
